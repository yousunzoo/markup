...
(function($){
  ...
  */
  ...
  // ---------------------------------------------------------
  // 선택자에서 순서를 가리키는 것 css:nth-child -> eq(순번) : 순번은 첫번째가 0
  
  boxList.eq(0).css({'backgroundColor':'#fff', 'padding':'10px'});
  boxList.eq(1).css({'backgroundColor':'#ddf', 'padding':'15px'});
  boxList.eq(2).css({'backgroundColor':'rgba(255,255,255,0.5)','padding':'10px'});

  // ---------------------------------------------------------
  // siblings : 선택한 요소를 제외한 나머지 형제 요소 = not()
  ...
  // jQuery.animate() 메소드 동작하지 않는 것들 : transition, animation, transform, borderRadius
})(jQuery);